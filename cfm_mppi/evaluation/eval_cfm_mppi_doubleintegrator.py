...
# MPPI
MPPI_SIGMA = torch.tensor([0.4, 0.4])
MPPI_LAMBDA = 0.1
...
state_trajectories = torch.zeros(
    [batch_ego.shape[0], 4, horizon + 1], dtype=torch.float32
)
control_trajectories = torch.zeros(
    [batch_ego.shape[0], 2, horizon], dtype=torch.float32
)

result_root = Path(os.environ.get("EVAL_RESULT_ROOT", "results"))
directory_path = result_root / f"{dataset}_db_{precision}"
directory_path.mkdir(parents=True, exist_ok=True)

n_scenarios = int(os.environ.get("EVAL_N_SCENARIOS", batch_ego.shape[0]))
recorder = InstrumentationRecorder(
    n_scenarios=n_scenarios, horizon=horizon, use_cuda=torch.cuda.is_available()
)
recorder.write_hyperparams(
    directory_path,
    {
        "SAFE_MARGIN": SAFE_MARGIN,
        "SAFE_COEF": SAFE_COEF,
        "GOAL_COEF": GOAL_COEF,
        "MPPI_SIGMA": MPPI_SIGMA.tolist(),
        "MPPI_LAMBDA": MPPI_LAMBDA,
        "HORIZON": HORIZON,
        "ODE_TIMES": ODE_TIMES,
        "ODE_TIMES2": ODE_TIMES2,
        "NOISE_LEVEL": NOISE_LEVEL.cpu().tolist(),
        "n_sample": n_sample,
        "D": D,
        "U_MIN": U_MIN.tolist(),
        "U_MAX": U_MAX.tolist(),
...
for idx in range(n_scenarios):
    ...
    if dataset == "ucy" or dataset == "sdd":
        ...
        goal = torch.tensor(
            [[6.0, 6.0]],
            dtype=torch.float32,
            device=device,  # Example goal position
        )
        ...
        pos_obs = torch.zeros(
            [1, n_hum, 2, horizon], dtype=torch.float32, device=device
        )
        vel_obs = torch.zeros(
            [1, n_hum, 2, horizon], dtype=torch.float32, device=device
        )
    ...
    flowmppi_solver = FlowMPPI(
        ...
        dim_state=4,
        dim_control=2,
        dynamics=doubleintegrator_dynamics,
    ...
    state = torch.zeros(1, 4).to(device)
    x_0 = torch.randn([n_sample, 2, 80], dtype=torch.float32, device=device)
    state_hist = torch.zeros([4, horizon + 1], dtype=torch.float32)
    ...
    for t in range(horizon):
            ...
            if t != 0:
                for i in range(n_hum):
                    others_states = torch.cat(
                        [
                            pos_obs[0, np.r_[0:i, i + 1 : n_hum], :, t - 1].cpu(),
                            state[:, :2].cpu(),
                        ],
                        dim=0,
                    )
                    control_history_sin = histories["ego_control_sin"].get()
                    others_controls = torch.cat(
                        [
                            vel_obs[0, np.r_[0:i, i + 1 : n_hum], :, t - 1].cpu(),
                            control_history_sin[:, :, -1].cpu(),
                        ],
                        dim=0,
                    )
                    humans[i].social_force_step(
                        others_states.numpy(), others_controls.numpy()
                    )
        ...
        state = doubleintegrator_dynamics(state, control_dyn, dt)
        ...
        noise = torch.randn(
            n_sample, controls_sin.shape[1], controls_sin.shape[2], device=device
        )
        ...
        control_history_sin = histories["ego_control_sin"].get()
        x_t = torch.cat(
            [control_history_sin.expand(n_sample, -1, -1) / 10, x_t], dim=-1
        )
    ...
    coll_val = int(collision.item()) if hasattr(collision, "item") else int(collision)
    ...
    recorder.end_scenario(
        ...
        scenario_metrics={
            "coll": coll_val,
            "dist": dist_val,
            "mean_step_ms": average_time * 1000.0,
            "n_obs": int(pos_obs.shape[1]),
        },
    )
...
recorder.dump_cell(directory_path)
print(f"[trace] dumped cell artifacts to {directory_path}", flush=True)
